...
const HolidayPackageCard: React.FC<{ pkg: HolidayPackage, onBookServiceClick: (serviceName:string) => void }> = ({ pkg, onBookServiceClick }) => (
    <div className="bg-gray-800 rounded-lg shadow-2xl overflow-hidden group border border-transparent hover:border-amber-400 transition-all duration-300 flex flex-col md:flex-row relative">
      <div className="absolute top-0 right-0 bg-red-600 text-white text-xs font-bold px-3 py-1 rounded-bl-lg rounded-tr-lg z-10">
        عرض خاص
      </div>
      <div className="md:w-1/3 h-48 md:h-auto overflow-hidden">
        <img src={pkg.imageUrl} alt={pkg.name} className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-500" />
      </div>
...
const HolidayPackages: React.FC<HolidayPackagesProps> = ({ onBookServiceClick }) => {
  return (
    <section id="holiday-packages" className="py-16 sm:py-20 bg-black">
